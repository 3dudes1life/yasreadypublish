...
export function runEpubPreflight({ project, storyLockOk = true } = {}) {
  ...
  const checks = [
    ...
    check('images', 'Image assets', imageCount === 0 ? 'pass' : 'error', imageCount === 0 ? 'No DOCX image assets need ebook packaging.' : `${imageCount} DOCX image asset(s) detected. v1.0 blocks EPUB export rather than silently omitting them.`),
    ...
    check('word-tables', 'Word tables', (project?.manuscript?.metadata?.tableCount || 0) ? 'warning' : 'pass', (project?.manuscript?.metadata?.tableCount || 0) ? `${project.manuscript.metadata.tableCount} Word table(s) were detected. Paragraph text is preserved, but table grid layout is not reproduced in EPUB v1.0.` : 'No Word table structures detected.'),
