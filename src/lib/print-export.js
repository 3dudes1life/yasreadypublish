...
function renderPage(page, design, project, blocksById) {
  ...
  const fragments = page.intentionalBlank ? '' : (page.fragments || []).map((fragment) => {
    if (fragment.kind === 'blank') {
      ...
      return `<div class="fragment blank" style="height:${h}in"></div>`;
    }
    ...
    if (fragment.isFinalPiece && design.paragraphGap && !['chapter-title', 'blank'].includes(fragment.kind)) style += `padding-bottom:${design.paragraphGap}in;`;
...
export function buildPrintMasterHtml({ project, preview, manuscriptHash = '' } = {}) {
...
<!-- Generated by YasReady Publish 0.9.0 at ${generatedAt}. Source hash: ${escapeHtml(manuscriptHash)} -->
</body>
</html>`;
}
