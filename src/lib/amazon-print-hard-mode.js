...
export function runAmazonPrintHardMode({ project, type = 'paperback', preview = null, interiorAudit = null, coverAudit = null } = {}) {
  ...
  checks.push(check('amazon-interior-fonts','Interior font embedding/substitution risk',pdfFonts?.status === 'error' ? 'error' : pdfFonts?.status === 'warning' ? 'warning' : pdfFonts?.status === 'pass' ? 'pass' : interiorAudit ? 'error' : 'warning',pdfFonts?.message || (interiorAudit ? 'Finished PDF font audit is unavailable.' : 'Pending finished interior PDF audit.')));
  ...
  const uploadedSpineArtwork = edition.coverMode === 'upload-art' && Boolean(edition.uploadedCoverArt);
  checks.push(check('amazon-spine-text','Spine text eligibility + safety',
    uploadedSpineArtwork ? (cover.spineTextAllowed ? 'pass' : 'error') : (!hasSpineText || cover.spineTextAllowed ? 'pass' : 'error'),
    uploadedSpineArtwork
      ? (cover.spineTextAllowed ? `Spine text is embedded in the uploaded full-wrap artwork; ${pageCount} pages is eligible for spine text. Final visual placement remains part of the YasReady/KDP visual proof.` : `Uploaded artwork contains a spine region, but spine text is not allowed at ${pageCount} pages for this geometry.`)
      : !hasSpineText ? 'No generated spine text is configured.' : cover.spineTextAllowed ? `Spine text is allowed at ${pageCount} pages; Cover Brain reserves at least ${Number(cover.spineSafeInset || 0.0625).toFixed(4)} in from each spine edge.` : `Spine text is not allowed at ${pageCount} pages for this cover geometry.`));
  ...
  checks.push(check('amazon-cover-pdf-safety','Cover PDF safety / flattening',securityErrors.length ? 'error' : coverAudit?.ready ? 'pass' : coverAudit ? 'error' : 'warning',securityErrors.length ? securityErrors.map((item)=>item.label).join(', ') : coverAudit?.ready ? 'Cover audit found no blocking security, annotation/form, transparency, or font-embedding issue.' : coverAudit ? 'Final cover technical audit is incomplete.' : 'Pending final cover PDF audit.'));
