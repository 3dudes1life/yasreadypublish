...
import { FULL_WRAP_ART_VERSION, analyzeFullWrapArtwork, normalizeFullWrapArtwork, renderFullWrapArtworkPdf } from './lib/full-wrap-art.js';
import {
...
import { capturePrintSetupState, planPrintSetupInvalidation, printProductionFingerprint } from './lib/print-state-invalidation.js';
import {
...
function renderPrintBrainSetup() {
  ...
  return `<article class="panel print-brain-panel">
    <div class="simple-page-head"><div><span class="simple-kicker">PRINT BRAIN · v1.0.37</span><h2>Make your ${type === 'hardcover' ? 'hardcover' : 'paperback'}.</h2><p>Choose the physical book. YasReady handles the KDP geometry and checks the combination against Amazon’s manufacturing rules.</p></div></div>
    ...
    <section class="print-barcode-choice">
      <div class="print-cover-choice-head"><div><span class="simple-kicker">BARCODE BRAIN · v1.0.37</span><h3>Lock the ISBN into the physical book.</h3><p>Barcode Brain runs before final spine math because the optional last-page ISBN barcode becomes a real physical page. One ISBN can drive the last interior page, back cover, and standalone barcode assets.</p></div><span class="mini-status ${isbnStatus.valid ? 'good' : 'needs'}">${isbnStatus.valid ? 'ISBN VALID' : 'ISBN NEEDED'}</span></div>
  ...
  const fragments = page.intentionalBlank
    ...
    : page.barcodePage
      ...
      if (matterSpec) {
        ...
        const matterWhiteSpace = ['matter-back-heading','matter-back-body'].includes(fragment.kind) ? 'pre-wrap' : 'normal';
        extra = `padding-top:${(matterSpec.paddingTopIn || 0) * px}px;padding-bottom:${(matterSpec.paddingBottomIn || 0) * px}px;font-size:${matterSize}px;line-height:${matterSpec.lineHeight};text-align:${matterSpec.alignment};font-weight:${matterSpec.bold ? 700 : 400};font-style:${matterSpec.italic ? 'italic' : 'normal'};white-space:${matterWhiteSpace};`;
...
function renderCoverBrain() {
  ...
  if (edition.coverMode === 'upload-pdf') {
    ...
    return `<section class="cover-brain-card uploaded-cover-card">
      ...
      ${barcodeIsbn.valid ? `<div class="barcode-asset-actions"><button class="btn ghost" id="downloadBarcodeSvg" type="button">Download ISBN barcode SVG</button><button class="btn ghost" id="downloadBarcodePng" type="button">Download 300-DPI PNG</button></div>` : ''}
      <div class="cover-brain-actions"><button class="btn ${uploadAudit.ready && report.checks.find((item)=>item.id==='barcode')?.status !== 'error' ? 'primary' : 'secondary'}" id="buildCoverPdf" type="button" ${uploadAudit.ready && report.checks.find((item)=>item.id==='barcode')?.status !== 'error' ? '' : 'disabled'}>${barcode.coverPlacement === 'yasready' ? 'Prepare Amazon/KDP cover PDF' : 'Download attached KDP cover PDF'}</button><button class="btn ghost" data-simple-step="style" type="button">← Change cover choice</button></div>
    </section>`;
  }
  ...
  return `<section class="cover-brain-card">
    <div class="cover-brain-head"><div><div class="eyebrow">COVER BRAIN + BARCODE BRAIN · v1.0.37</div><h3>${escapeHtml(editionLabel(type))} cover</h3><p>Final page count drives the spine and full-wrap canvas. YasReady reserves the barcode zone and keeps every critical element inside KDP safety.</p></div><div class="cover-brain-dims"><strong>${g.width.toFixed(4)} × ${g.height.toFixed(4)}</strong><span>in full cover</span><small>Spine ${g.spineWidth.toFixed(4)} in${g.exact ? '' : ' · estimate'}</small></div></div>
    ${state.coverBrainMessage ? `<div class="notice info">${escapeHtml(state.coverBrainMessage)}</div>` : ''}
...
async function savePrintBrainSetup(useRecommended = false) {
  ...
  const productionChangedByForm = setupBefore.production !== printProductionFingerprint(production, type);
  ...
  let barcode = normalizeBarcodeBrain({
    ...
    coverPlacement:document.querySelector('#printBrainCoverBarcode')?.value || 'amazon',
  });
  ...
  // Do not silently re-style the interior while the user is merely accepting a
  // cover. Print Brain only re-applies physical design when its production inputs
  // actually changed (or Recommended was explicitly requested).
  if (productionChangedByForm || useRecommended) {
  ...
  if (setupInvalidation.interiorChanged) {
    // A real pagination-affecting change invalidates the complete physical package.
    ...
    state.preview = null;
    state.simpleStep = 'preview';
  } else if (setupInvalidation.coverChanged) {
    // Cover-only acceptance preserves the current pagination, preflight and finished
    // interior audit. Return to Export so the user can manufacture only the new cover.
    edition.lastCoverAudit = null;
    state.coverPdfReport = null;
    state.simpleStep = state.preview ? 'export' : 'preview';
  }
  state.finalCheck = null;
  state.project.updatedAt = new Date().toISOString();
...
async function buildCurrentCoverPdf() {
  ...
  if (edition.coverMode === 'upload-art') {
    ...
    try {
      ...
      if (!packaged.audit?.ready || packaged.generatorVersion !== FULL_WRAP_ART_VERSION || packaged.visualQuality?.ready !== true) throw new Error('Cover Brain blocked this manufactured cover because the current visual-quality audit did not pass.');
      const sha256 = await sha256Hex(packaged.bytes);
