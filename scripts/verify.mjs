...
for (const marker of [
  "const VERSION = '1.0.18'", 'Run Final Check', 'Download Project Backup', 'Kindle / eBook', 'Download KDP EPUB',
  ...
  if (!main.includes(marker)) throw new Error(`1.0.18 production workspace is missing: ${marker}`);
}
...
if (!project.includes("appVersion: '1.0.18'") || !project.includes('version: 25') || !project.includes('ensureEditions(project)') || !project.includes('ensurePresentationOverrides(project)')) {
  throw new Error('Project app version was not migrated to 1.0.18 while preserving schema 25 safety state.');
}
...
for (const marker of ['chapter-layout-${layout}','chapter-label','chapter-name','splitChapterHeading']) {
  if (!epub.includes(marker)) throw new Error(`1.0.18 Kindle chapter renderer is missing: ${marker}`);
}
...
for (const marker of ['Step 1 · Book','Step 2 · Style','Step 3 · Preview','Step 4 · Export','Advanced Tools','simpleKindleExport','simpleBookStyle']) {
  if (!main.includes(marker)) throw new Error(`1.0.18 Simple Mode is missing: ${marker}`);
}
...
console.log('YasReady Publish v1.0.18 static verification passed.');
